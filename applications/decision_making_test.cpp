...
int main(int argc, char** argv) {
    ...
    network.allToAll<hummus::Exponential>(pixel_grid, hidden_layer, 1, hummus::Normal(0.08, 0.02, 10, 3), 60); // all-to-all connection from the pixel grid to the hidden layer
    network.lateralInhibition<hummus::Exponential>(hidden_layer, 1, hummus::Normal(-1, 0, 0, 1), 100, 60); // lateral inhibition within neurons in the hidden layer
    
    /// Reading data
    hummus::DataParser dataParser;
    auto trainingData = dataParser.readTextData("../../data/nmnist_training.txt");
    auto testData = dataParser.readTextData("../../data/nmnist_test.txt", 1000);
