...
int main(int argc, char** argv) {
    ...
    if (cadence) {
        ...
        auto output = network.make_layer<hummus::ULPEC_LIF>(1, {}, 0, 5e-12, 0, 0, 12e-9, 0, 725, true, 0.5, 10, 1.5, 1.2);
        ...
        network.all_to_all<hummus::Memristor>(input, output, 1, hummus::Normal(1e-5), 100);
        ...
        auto training_database = parser.generate_nmnist_database("/Users/omaroubari/Documents/Education/UPMC - PhD/Datasets/es_N-MNIST/Train", 1, {"5", "6", "9"});
        
        /// generating N-MNIST test database
        auto test_database = parser.generate_nmnist_database("/Users/omaroubari/Documents/Education/UPMC - PhD/Datasets/es_N-MNIST/Test", 1, {"5", "6", "9"});
        
        auto& ulpec_stdp = network.make_addon<hummus::ULPEC_STDP>(0.1, -0.1, -1.2, 1.2, 1e-7, 1e-9);
        ...
        auto output = network.make_layer<hummus::ULPEC_LIF>(100, {&ulpec_stdp}, 10, 1e-12, 1.2, 0, 100e-12, 0, 15, true, 0.5, 10, 1.5, 1); /// 100 ULPEC_LIF neurons
        auto decision_layer = network.make_decision<hummus::Decision_Making>(training_database.second, 10, 60, 0, {});

        auto& g_maps = network.make_addon<hummus::WeightMaps>("ulpec_g_maps.bin", 1);
        ...
        network.all_to_all<hummus::Memristor>(pixel_grid, output, 1, hummus::Uniform(1e-9, 1e-7, 0, 0, false), 100);
