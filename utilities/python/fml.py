...
class FML2(torch.nn.Module):
	def __init__(self, input_dim, output_dim):
		super(FML2, self).__init__()
		H=200
		self.linear11 = torch.nn.Linear(input_dim, H)
		# self.activation11 = torch.nn.Sigmoid()
		self.linear12 = torch.nn.Linear(input_dim, H)
		# self.activation12 = torch.nn.ReLU()
		self.linear2 = torch.nn.Linear(H, output_dim)
		self.activation2 = nn.LogSoftmax(dim=1)

	def forward(self, x):
		outputs1 = self.linear11(x)
		outputs2 = self.linear12(x)
		# outputs = self.activation1(outputs)
		outputs = self.linear2(outputs1*outputs2)
...
acc = []
for k in dpts[1:]:
	lreg = FML(n_in=100,n_out=3, epochs=70)
